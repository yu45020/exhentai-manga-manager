...
- `resultLists` 可能为undefined [code](link)
- `callback` 可能为undefined [code](link)
- `collectionList` 可能不是数组 [code](link), [code2](link)

### 其他

- 在 `getBookInfoFromHentag` 中，使用 `category: categoryOption.value[data.category-1]`（索引从0开始）。
...
- **设计：**
  ...
  - 文件可移动到任意文件夹，不限于库文件夹。
- **补丁：** `MoveFileDialog.vue` [commit 14](link)

## UI微调

- 缺失文件分配“缺失”状态 [commit 6](link)
- E-Hentai风格分类标签，保留 `tag-failed` 和 `non-tag` [commit 9](link)
- “缺失”分类用于缺失文件。每次扫描都验证文件存在。
- `No Tag Only` 过滤器 [commit 10](link)
- 书籍详情（编辑标签）列出默认标签 [commit 11](link)
- 鼠标前进/后退导航 [commit 12](link)：
  - `home` UI：
    - a. 后退按钮：
      - 当前页为首页时重置（同原行为）
      - 否则返回上一页
